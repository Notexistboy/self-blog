...
const continued = async <T extends (page?: number) => Promise<unknown[]>>(fn: T, page = 1) => {
...
(async () => {
  ...
  try {
    const [labelsData, issuesData, userData] = await Promise.all([continued(labels), continued(issues), user()]);
