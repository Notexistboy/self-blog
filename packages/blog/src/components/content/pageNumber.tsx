...
export const PageNumber: FC<Props> = ({ pagingData, page }) => {
  const arr = useMemo(() => {
    ...
    switch (page) {
      ...
      case count - 3:
      case count - 4:
        place = [1, 'pre', count - 6, count - 5, count - 4, count - 3, count - 2, count - 1, count];
        break;

      default:
        place = [1, 'pre', page - 2, page - 1, page, page + 1, page + 2, 'next', count];
        break;
    }
    ...
    <div>
      <ul>
        {arr.map((item, index) => {
          ...
          return (
            <li className={page === item.page ? 'uk-active' : ''} key={item.url}>
